...
module Group_IO
   ...
   implicit none
   integer, parameter :: STUD_AMOUNT   = 3
   integer, parameter :: SURNAME_LEN   = 14
   integer, parameter :: NUMS_LEN  = 10

   ! Структура данных для хранения данных о студенте.
   type student
      character(SURNAME_LEN, kind=CH_)    :: Surname              = ""
      character(NUMS_LEN, kind=CH_)   :: Num             = ""
   end type student
...
contains
   ...
   function Read_class_list(Data_File) result(Group)
      ...
      recl = ((SURNAME_LEN + NUMS_LEN)*CH_) * STUD_AMOUNT
