...
program lab_1_2
   ...
   call Output_class_list(output_file, Surnames, Initials, "rewind","ordered list:")
      
contains
   subroutine Read_class_list(Input_File, Surnames, Initials)
   ...
   subroutine Output_class_list(Output_File, Surnames, Initials, Position,List_name)
      character(*)         Output_File, Position, List_name
      character(kind=CH_)  Surnames(:, :), Initials(:, :)
      intent (in)          Output_File, Surnames, Initials, Position
   ...
   subroutine Sort_ins(Surnames,Initials)
        ...
        integer :: i,j
        character(kind=CH_ ) :: tmpInitials(INITIALS_LEN)
        ...
        do i = 2, STUD_AMOUNT
          ...
          do while (j>1 .and.Condition(Surnames,Initials,tmpSurnames,tmpInitials,j))
...
pure logical function Condition( Surnames, Initials,tmpSurnames,tmpInitials, j)
       character(kind=CH_)  Surnames(:, :), Initials(:, :),tmpSurnames(:),tmpInitials(:)
       integer              j
       intent (in) :: Surnames, Initials,tmpSurnames, tmpInitials, j
 
       Condition = .false.
       if (GT(Surnames(:, j-1), tmpSurnames)) then
             Condition = .true.
       else if (All(Surnames(:, j-1) == tmpSurnames) .and. GT(Initials(:, j-1),     tmpInitials)) then
            Condition = .true.
       end if
